...
export function CourseCard({ slug, title, description, thumbnail, lessons, lessonsLabel = "lessons" }: Props) {
  return (
    <Link href={`/courses/${slug}`} className="group overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#111111] transition hover:-translate-y-1 hover:border-gold/40 sm:rounded-[1.75rem]">
      ...
      <div className="space-y-3 p-5 sm:p-6">
        <p className="text-[11px] uppercase tracking-[0.28em] text-gold sm:text-xs sm:tracking-[0.3em]">{lessons} {lessonsLabel}</p>
